...
const GoalsListSection = () => {
  ...
  // which goal is expanded to show its tasks (only one at a time)
  const [openGoalId, setOpenGoalId] = useState(null);

  // component to load & render tasks for a specific goal (safe hook usage)
  const GoalTaskList = ({ goalId }) => {
    const { data: tasksForGoal = [], isLoading, isError } = useQuery({
      ...
      queryFn: async () => {
        const { data, error } = await supabase
          .from("tasks")
          .select(
            "id, description, pomodorosNumber, pomodorosDone, isToday, isFinished"
          )
          .eq("goal_id", goalId)
    ...
    if (isLoading)
      return <div className="text-sm text-gray-500 mt-2">Loading tasks…</div>;
    if (isError)
      return <div className="text-sm text-red-600 mt-2">Failed to load tasks.</div>;
    if (!tasksForGoal.length)
      return <div className="text-sm text-gray-600 mt-2">No tasks for this goal.</div>;
      ...
      <ul className="mt-2 space-y-2">
        {tasksForGoal.map((t) => (
          <SortableTaskItem
            ...
            compact={true}
            showGoalName={false}
            showDragHandle={false} // disable drag handle/dots for goal list
          />
        ))}
      </ul>
    );
      ...
      {goals.length === 0 ? (
        ...
        <ul className="space-y-3">
          {goals.map((g) => (
            <li
              key={g.id}
              className="border border-[#f4e1e6] rounded-lg px-3 py-2 bg-white"
            >
               ...
               <div className="flex-1">
                 {editingId === g.id ? (
                   <div className="flex gap-2 items-center">
                     ...
                     >
                       {editGoalMutation.isLoading ? "Saving…" : "Save"}
                     </button>
                     <button
                       onClick={(e) => {
                         e.stopPropagation();
                   ...
                   <div>
                     {/* only the goal name is the toggle "button" */}
                     <div
               ...
               <div className="flex items-center gap-2 ml-4">
                 <button
                   ...
                   onClick={(e) => {
                     ...
                     // toggle inline add area for this goal
              ...
              {/* inline area: tasks for this goal (only rendered when open) */}
              {openGoalId === g.id && (
              ...
              {/* inline add-task panel under the goal card (toggles independently) */}
              {addTaskGoalId === g.id && (
       ...
       {deleteConfirmId && (
         ...
         >
           ...
           >
             <h4 className="text-lg font-semibold text-[#4b2e2e]">Delete goal</h4>
             <p className="text-sm text-gray-600 mt-2">
               Are you sure you want to permanently delete this goal and its tasks?
             </p>
             <div className="mt-4 flex justify-end gap-3">
               ...
               <button
                 ...
                 disabled={deleteGoalMutation.isLoading}
               >
                 {deleteGoalMutation.isLoading ? "Deleting..." : "Delete"}
               </button>
             </div>
           </div>
         </div>
